...
public class Text {
     ...
     * Parses a number.
     *
     * @return a parser that parses a number
     */
    ...
    public static Parser<Character, Integer> integer() {
        return choice(List.of(
                chr('+').skipLeftThen(Parser.pure(true)),
                chr('-').skipLeftThen(Parser.pure(false)),
                Parser.pure(true))
     ...
     */
    public static final Parser<Character, Boolean> sign = choice(List.of(
            chr('+').skipLeftThen(Parser.pure(true)),
            chr('-').skipLeftThen(Parser.pure(false)),
            Parser.pure(true)
    ));
    ...
    public static final Parser<Character, Double> dble = sign.then(ulng)
            .then((chr('.').skipLeftThen(floating)).optional())
            .then(expnt.optional())
            .map((sn, i, f, exp) -> {
